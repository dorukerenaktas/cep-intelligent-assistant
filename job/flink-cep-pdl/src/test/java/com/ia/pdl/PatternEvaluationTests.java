...
public class PatternEvaluationTests {
    ...
    @Test
    public void shouldEvaluateContains() throws Exception {
        TestEvent event = new TestEvent();
        event.setAttribute("attribute", "abcdefg");
        event.setEventType("A");

        executeTest("A(attribute >< 'cde')", Lists.newArrayList(event));
    ...
    @Test
    public void containsShouldIgnoreCase() throws Exception {
        TestEvent event = new TestEvent();
        event.setAttribute("attribute", "AbCdEfG");
        event.setEventType("A");
    ...
    @Test
    public void containsShouldProcessCorrelation() throws Exception {
        TestEvent event = new TestEvent();
        event.setEventType("A");
        event.setAttribute("attribute", "testabc");
        event.setAttribute("correlation_id", 10);
        ...
        executeTest("A(attribute >< 'sta') -> B(attribute >< 'c2' and correlation_id=A.correlation_id)", Lists.newArrayList(event, event2));
    ...
    private void shouldEvaluateWithinTimeWindowBase(Long secondEventTimeStamp, int expected) throws Exception {
        ...
        PatternStream<Event> patternStream = Pdl.withStrictEventTypeMatching().compile(pattern, eventDataStream);
    ...
    private void executeTest(String pattern, List<Event> data) throws Exception {
        ...
        PatternStream<Event> patternStream = Pdl.compile(pattern, eventDataStream);
